<div class="p-6 bg-white shadow-md rounded-lg">
    <div class="grid grid-cols-3 gap-4">
        ...
        <div class="col-span-1 p-4 border rounded bg-gray-100">
            <button wire:click="innemen({{ $geselecteerdApparaat }})" class="bg-blue-500 text-white px-4 py-2 rounded-l-full border-r-2 border-blue-700 hover:bg-blue-600">
            ...
            <table class="min-w-full">
                ...
                @foreach($apparaten as $apparaat)
                    <tr wire:click="selectApparaat({{ $apparaat->id }})" class="{{ $geselecteerdApparaat == $apparaat->id ? 'bg-green' : '' }}">
        ...
        <!-- Ingenomen Section -->
        <div class="col-span-1 p-4 border rounded bg-gray-100">
            <button wire:click="terugnemen({{ $geselecteerdApparaat }})" class="bg-blue-500 text-white px-4 py-2 rounded-r-full border-l-2 border-blue-700 hover:bg-blue-600">
                <i class="fas fa-arrow-left"></i>
            </button>
            <button wire:click="allesTerugnemen" class="bg-blue-500 text-white px-4 py-2 rounded-r-full border-l-2 border-blue-700 hover:bg-blue-600">
            ...
            <table class="min-w-full">
                ...
                <tbody>
                @foreach($ingenomen as $index => $apparaat)
                    <tr wire:click="selectIngenomenApparaat({{ $index }}, {{ $apparaat->id }})" class="{{ $geselecteerdIngenomenApparaat['index'] === $index ? 'bg-green' : '' }}">
                        <td class="border px-4 py-2">{{ $apparaat->naam }}</td>
                        <td class="border px-4 py-2">€ {{ number_format($apparaat->vergoeding, 2, ',', '') }}</td>
                    </tr>
                @endforeach
