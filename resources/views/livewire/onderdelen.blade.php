    ...
    <table class="min-w-full bg-white">
        ...
        <tr>
            ...
            <th class="py-2 px-3 border-b text-center">
                <a href="{{route('onderdeeltoevoegen')}}" class="no-print fas fa-plus-circle text-green-500" title="Toevoegen"></a>
                <i wire:click="editSelectedRow" class="no-print {{ is_null($selectedRow) ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer' }} fas fa-pencil-alt text-yellow-500" title="Bewerken"></i>
                <i wire:click="deleteSelectedRow" class="no-print {{ is_null($selectedRow) ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer' }} fas fa-trash-alt text-red-500" title="Verwijderen"></i>
                <button onclick="window.print();" class="no-print fas fa-print text-blue-500 cursor-pointer" title="Afdrukken"></button>
            </th>
        </tr>
        </thead>
        <tbody>
